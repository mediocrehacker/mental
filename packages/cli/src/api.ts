...
export const depressionCheckup = async (
  providers: PHQProviders,
  phqContract: DeployedPHQContract): Promise<FinalizedTxData> => {
  logger.info('checkup...');
  const finalizedTxData = await phqContract.callTx.depressionCheckup();
  logger.info(`Transaction ${finalizedTxData.public.txId} added in block ${finalizedTxData.public.blockHeight}`);
  return finalizedTxData.public;
};

export const displayPHQValue = async (
  providers: PHQProviders,
  phqContract: DeployedPHQContract,
): Promise<{ phqValue: bigint | null; contractAddress: string }> => {
  const contractAddress = phqContract.deployTxData.public.contractAddress;
  const phqValue = await getPHQLedgerState(providers, contractAddress);
  if (phqValue === null) {
    logger.info(`There is no phq contract deployed at ${contractAddress}.`);
  } else {
    logger.info(`Current phq value: ${Number(phqValue)}`);
  }
  return { contractAddress, phqValue };
};
